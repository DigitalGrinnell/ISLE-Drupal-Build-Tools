...
drush -u 1 -y vset islandora_solr_collection_search_request 'http:/isle-apache-ld:8080/solr/select?q=RELS_EXT_hasModel_uri_s:(info/:fedora/islandora/:collectionCModel)&fl=PID,fgs_label_s&fq=PID:(grinnell/:*%20OR%20faulconer-art/:*)&wt=php&rows=999'
...
drush -u 1 -y vset islandora_solr_url 'repositoryx.grinnell.edu:8080/solr'
drush -u 1 -y vset islandora_solr_use_ui_qf 0

drush -u 1 -y vset islandora_solr_primary_display_table:
  weight:
    default: '-10'
    grid: '-8'
    table: '-7'
...
drush -u 1 -y vset islandora_solr_secondary_display:
  csv: csv
  rss: 0
